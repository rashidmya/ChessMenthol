...
// app/src/tests/engine.integration.test.ts
...
// jsdom). ONE engine instance is shared across all tests: the wasm module cannot
// be instantiated twice in the same process (a second initEngine() throws
// `LinkError: Import #17 ... requires a callable`), so we load once in beforeAll
// and reuse it. If the real engine fails to load, these gate tests FAIL loudly —
// they never silently skip, so a green run means the engine genuinely ran.
...
describe('real stockfish.wasm via Node (AnalysisSession integration)', () => {
  ...
  it('supersedes an in-flight search: onDone fires once, only for the new position', async () => {
    ...
    // only the new search completes. This is the real-engine gate for the drain fix.
