...
> **Implemented design differs from the draft below — final version committed `80c3c3a` (supersedes the `ef24773` interim).** Two corrections; the committed `session.ts`/`session.test.ts` are authoritative.
>
> **(1) Synchronize on `bestmove`, not `readyok` (Critical, caught in final review and verified against the bundled engine).** The draft assumed an `isready`/`readyok` "barrier" would order a stopped search's trailing `bestmove` *before* the new search's go. That is FALSE: a real UCI engine answers `isready` immediately, even mid-search, so the empirical order after `stop`+`isready` is **`readyok` → `bestmove`** (measured on `stockfish-18-lite-single`). The barrier therefore fired `onDone` spuriously and dropped the new search on every supersede — the core "re-analyze while the engine is thinking" flow. The final state machine is `idle → searching → draining`: on supersede/stop it sends `stop` and **waits for the stale `bestmove`** before launching the queued search (or going idle), matching how python-chess waits for the stopped bestmove. `isready` is dropped entirely from the per-search path. A real-engine supersede integration test now gates this (the bug slipped through because the original tests only covered natural completion + a `FakeEngine` that emitted `bestmove` *before* `readyok`).
>
> **(2) `launch()` primes `this.lastEmit = this.now() - this.throttleMs`** (not `0`) so the first `info` of a search always emits on the leading edge regardless of the clock epoch. `loadStockfish` also gained an init timeout (rejects + tears down the worker) so a failed load surfaces an error instead of hanging.
